...
export default function Index() {
  if (typeof document !== "undefined")
    document.title = "Will It Rain On My Parade?";
  const navigate = useNavigate();
  const { user, loading } = useAuth();
  const [location, setLocation] = useState("");
  const [datetime, setDatetime] = useState("");
  const [selected, setSelected] = useState<ConcernOption[]>([
    "Very Wet",
    "Very Uncomfortable",
  ]);
  ...
  useEffect(() => {
    ...
    if (
      inputRef.current &&
      (isNewSearch || !user || !user.preferences?.preferred_city)
    ) {
    ...
    if (
      ...
      const redirectTimer = setTimeout(() => {
        ...
        if (
          ...
          console.log("Preferred city:", user.preferences.preferred_city);
          console.log(
            "Redirecting user with preferred city:",
            user.preferences.preferred_city,
          );
    ...
    <div className="relative overflow-hidden">
      ...
      >
        ...
        <div className="container relative z-10 mx-auto max-w-5xl px-4 py-20 text-center md:py-28">
          ...
          >
            Planning a hike, vacation, or day on the lake? Check the likelihood
            of very hot, very cold, very windy, very wet, or very uncomfortable
            conditions for your chosen time and place.
          </motion.p>
          ...
          >
            ...
            <div className="mt-4">
              <motion.button
                type="submit"
                className="h-12 px-6 rounded-md bg-primary text-primary-foreground"
