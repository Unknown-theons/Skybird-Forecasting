...
const cityWeatherPatterns: Record<
  string,
  {
    baseTemp: number;
    tempRange: number;
...
export function generateWeatherData(city: string): WeatherData {
  ...
  const pattern = cityWeatherPatterns[normalizedCity] ||
    cityWeatherPatterns[city] || {
      baseTemp: 16,
      tempRange: 10,
      humidityBase: 60,
      windBase: 8,
  ...
  const seed =
    Array.from(city).reduce((a, c) => a + c.charCodeAt(0), 0) +
  ...
  const temperature =
    pattern.baseTemp + tempVariation + (rand() - 0.5) * pattern.tempRange;
  const humidity = Math.max(
    30,
    Math.min(90, pattern.humidityBase + (rand() - 0.5) * 30),
  );
  const windSpeed = Math.max(2, pattern.windBase + (rand() - 0.5) * 8);
  const rainProbability = Math.max(
    0,
    Math.min(100, pattern.rainBase + (rand() - 0.5) * 40),
  );
  ...
  const forecast = Array.from({ length: 12 }, (_, i) => {
    ...
    const hourTemp =
      ...
      (rand() - 0.5) * pattern.tempRange;
    const hourHumidity = Math.max(
      30,
      Math.min(90, pattern.humidityBase + (rand() - 0.5) * 30),
    );
    const hourWind = Math.max(2, pattern.windBase + (rand() - 0.5) * 8);
    const hourRain = Math.max(
      0,
      Math.min(100, pattern.rainBase + (rand() - 0.5) * 40),
    );
    const hourComfort = Math.max(
      0,
      Math.min(
        100,
        100 -
